...
CREATE TABLE IF NOT EXISTS `university`.`speciality` (
  `speciality_code` VARCHAR(45) NOT NULL,
  `speciality_name` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`speciality_code`),
  UNIQUE INDEX `speciality_code_UNIQUE` (`speciality_code` ASC) )
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `university`.`group` (
  `group_name` VARCHAR(45) CHARACTER SET 'armscii8' NOT NULL,
  `speciality_code` VARCHAR(50) NOT NULL,
  PRIMARY KEY (`group_name`),
  INDEX `codespec_idx` (`speciality_code` ASC) ,
  UNIQUE INDEX `group_name_UNIQUE` (`group_name` ASC) ,
  CONSTRAINT `codespec`
    FOREIGN KEY (`speciality_code`)
    REFERENCES `university`.`speciality` (`speciality_code`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;
